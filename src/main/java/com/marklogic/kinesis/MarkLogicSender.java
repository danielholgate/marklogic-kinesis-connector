...
public class MarkLogicSender {
	...
	public MarkLogicSender(KinesisConnectorForMarkLogicConfiguration config) {
		this._client = new MarkLogicJavaClientManager(config.MARKLOGIC_HOST, config.MARKLOGIC_PORT,
				config.MARKLOGIC_USER, config.MARKLOGIC_PASSWORD);
	...
	public List<MarkLogicMessageModel> sendToMarkLogic(List<MarkLogicMessageModel> documents) {
		...
		if (failedDocuments.size() > 0) {
			LOG.error("Failed to insert " + failedDocuments.size() + "/" + documents.size()
	...
	public boolean sendToMarkLogic(MarkLogicMessageModel document) {
		return this._client.insertDocument(document.getDocType(), this._collection, document.getUrl(),
				document.getData());
	}
=======
public class MarkLogicSender
